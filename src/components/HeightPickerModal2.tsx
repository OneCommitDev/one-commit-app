...
import {  Modal,  View,  Text,  TouchableOpacity,  Dimensions,  KeyboardAvoidingView,  Platform,} from 'react-native';
...
type HeightPickerModalProps = {
  ...
  onSave: (feet: number, inches: number) => void;
  title?: string;
  initialFeet?: number;  // <-- NEW
  initialInches?: number; // <-- NEW
};
...
export const HeightPickerModal2: React.FC<HeightPickerModalProps> = ({
  ...
  const [feetIndex, setFeetIndex] = useState(
    Math.max(0, feetRaw.indexOf(initialFeet))
  );
  const [inchIndex, setInchIndex] = useState(
    Math.max(0, inchRaw.indexOf(initialInches))
  );

  useEffect(() => {
    if (visible) {
      setFeetIndex(Math.max(0, feetRaw.indexOf(initialFeet)));
      setInchIndex(Math.max(0, inchRaw.indexOf(initialInches)));
    }
  }, [visible, initialFeet, initialInches]);
